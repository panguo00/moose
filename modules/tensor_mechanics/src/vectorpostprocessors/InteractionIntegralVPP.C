...
#include "InteractionIntegralVPP.h"
#include "MooseMesh.h"
...
MooseEnum
InteractionIntegralVPP::sifModeType()
{
...
template <>
InputParameters
validParams<InteractionIntegralVPP>()
{
  ...
  params.addParam<MooseEnum>("q_function_type",
                             ...
                             "The method used to define the integration domain. Options are: " +
                                 InteractionIntegralVPP::qFunctionType().getRawNames());
  params.addRequiredParam<MooseEnum>("sif_mode",
                                     InteractionIntegralVPP::sifModeType(),
                                     "Stress intensity factor to calculate. Choices are: " +
                                         InteractionIntegralVPP::sifModeType().getRawNames());

  return params;
}

InteractionIntegralVPP::InteractionIntegralVPP(const InputParameters & parameters)
  : ElementVectorPostprocessor(parameters),
    ...
    _interaction_integral(declareVector("II_" + Moose::stringify(_ring_index))) //BWSTODO figure out name
{
  if (!hasMaterialProperty<RankTwoTensor>("stress"))
    mooseError("InteractionIntegralVPP Error: RankTwoTensor material property 'stress' not found. "
  ...
  if (!hasMaterialProperty<RankTwoTensor>("elastic_strain"))
    mooseError("InteractionIntegralVPP Error: RankTwoTensor material property 'elastic_strain' not "
  ...
  if (_has_temp && !_total_deigenstrain_dT)
    mooseError("InteractionIntegralVPP Error: To include thermal strain term in interaction integral, "
  ...
  if (_ndisp != _mesh.dimension())
    mooseError("InteractionIntegralVPP Error: number of variables supplied in 'displacements' must "
...
void
InteractionIntegralVPP::initialSetup()
{
  _treat_as_2d = _crack_front_definition->treatAs2D();
}

void
InteractionIntegralVPP::initialize()
{
...
Real
InteractionIntegralVPP::computeQpIntegral(const unsigned int crack_front_point_index, const Real scalar_q, const RealVectorValue & grad_of_scalar_q)
{
...
void
InteractionIntegralVPP::execute()
{
...
void
InteractionIntegralVPP::finalize()
{
  ...
  for (unsigned int i = 0; i < _interaction_integral.size(); ++i)
  {
    ...
    if (_sif_mode == SifMethod::T && !_treat_as_2d)
      _interaction_integral[i] +=
        _poissons_ratio *
        _crack_front_definition->getCrackFrontTangentialStrain(i); //BWSTODO: this might not be parallel consistent
...
void
InteractionIntegralVPP::threadJoin(const UserObject & y)
{
  const InteractionIntegralVPP & uo = static_cast<const InteractionIntegralVPP &>(y);
...
void
InteractionIntegralVPP::computeAuxFields(RankTwoTensor & aux_stress, RankTwoTensor & grad_disp)
{
...
void
InteractionIntegralVPP::computeTFields(RankTwoTensor & aux_stress, RankTwoTensor & grad_disp)
{
