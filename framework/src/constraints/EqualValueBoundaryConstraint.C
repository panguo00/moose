...
EqualValueBoundaryConstraint::updateConstrainedNodes()
{
  ...
  else if ((_slave_node_ids.size() == 0) && (_slave_node_set_id != "NaN"))
  {
    ...
    {
      if ((*in != _master_node_vector[0]) && (_mesh.node(*in).processor_id() == _subproblem.processor_id()))
  ...
  else if ((_slave_node_ids.size() != 0) && (_slave_node_set_id == "NaN"))
  {
    ...
    {
      if ((_mesh.node(*its).processor_id() == _subproblem.processor_id()) && (*its != _master_node_vector[0]))
